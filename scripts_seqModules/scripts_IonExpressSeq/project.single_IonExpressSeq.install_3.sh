...
then
	...
	then
		...
		java -Xmx2g -jar $picardDirecgory AddOrReplaceReadGroups INPUT=$projectDirectory"data.temp.bam" OUTPUT=$projectDirectory"data.bam" RGID=1 RGLB=1 RGPL=ILLUMINA RGPU=1 RGSM=SM;
		cd $currentDir;
